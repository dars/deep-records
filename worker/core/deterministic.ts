...
export function handleDeterministicSceneTransition(
  ...
  // 阿陽登場後（門外敲門、樓梯間對峙或已進屋），移動與逃跑一律由模型敘事——
  // 罐頭轉場文字會忽略他的存在；場景切換仍由模型的 nextSceneId 經 validateNextSceneId 守門。
  if (hasOfficerArrived(state)) {
    return undefined
  }
...
export function handleDeterministicInvestigationAction(
  ...
  if (
    ...
    return {
      ...
      narration: [
        '你走到一樓入口旁那排老舊金屬信箱前。幾個信箱用奇異筆寫著姓氏，褪色廣告貼紙貼在鏽斑旁邊，被雨氣泡得邊角微微翹起。標著「4樓之2」的那一格，蓋子上還補寫了一個褪色的「林」字。',
